...
(() => {
  ...
  // ==========================================================================
  const THEME = {
    // ----- Stream identity (panels hidden; values unused) -----
    streamTag:       '',
    crestSymbol:     '',
    streamTitle:     '',
    streamSubtitle:  '',
    ...
    // ----- Fonts -----
    // Zen Kaku Gothic New 900 = chunky JP sans matching the MV's text-slam aesthetic.
    // Rubik 800/900 = matching chunky sans for EN.
    fontsHref: 'https://fonts.googleapis.com/css2?family=Zen+Kaku+Gothic+New:wght@900&family=Rubik:ital,wght@0,800;0,900;1,800&family=M+PLUS+1:wght@900&display=swap',
    fontDisplay: '"Rubik", system-ui, sans-serif',
    fontBody:    '"Rubik", system-ui, sans-serif',
    fontSerif:   '"Rubik", system-ui, sans-serif',
    fontJP:      '"Zen Kaku Gothic New", "M PLUS 1", system-ui, sans-serif',

    // ----- Color palette (panels unused; only lyric fallback values matter) -----
    cream:      '#ffffff',
    accent:     '#5eb8e7',
    accentDeep: '#2580b8',
    accentInk:  '#1a3a5c',
    ...
    panelBackground: `
      ...
      linear-gradient(172deg, rgba(254,252,251,0.94) 0%, rgba(254,236,236,0.92) 45%, rgba(250,213,220,0.9) 100%)
    `,
    panelBorder:      '1px solid rgba(249, 119, 151, 0.3)',
    panelRadius:      '24px',
    panelShadow:      '0 30px 64px -28px rgba(168, 54, 90, 0.55)',

    // ----- Side tabs (panel collapse buttons) -----
    tabBackground: 'linear-gradient(180deg, #ffa4bc, #d04d74)',
    tabTextColor:  '#fff',
    tabShadow:     '0 6px 20px -6px rgba(208, 77, 116, 0.5)',

    // ----- Now-playing card -----
    nowCardBackground: 'linear-gradient(140deg, rgba(255,255,255,0.82), rgba(254,224,224,0.7))',
    nowCardBorder:     '1px solid rgba(249, 119, 151, 0.26)',
    nowCardShadow:     '0 10px 22px -14px rgba(208,77,116,0.35), inset 0 1px 0 rgba(255,255,255,0.75)',
    nowFillGradient:   'linear-gradient(90deg, #ffa4bc 0%, #f97797 50%, #d04d74 100%)',

    // ----- Setlist rows -----
    rowHoverBg:   'rgba(249, 119, 151, 0.1)',
    rowActiveBg:  'linear-gradient(100deg, rgba(249,119,151,0.24), rgba(249,119,151,0.08))',
    rowActiveBar: '#d04d74',  // left border on active row

    // ----- Ctrl buttons (Skip talking / Offset / Hide lyrics) -----
    // Background of each button. For dark themes, use a dark translucent value.
    ctrlBackground: 'rgba(255, 255, 255, 0.55)',

    // ----- Lyrics -----
    // White fills with deep-navy halo — echoes the MV's white text on sky-blue.
    // The video is ~solid sky-blue throughout, so we tune for THAT, not anime-frame unpredictability.
    lyricColorEN: '#ffffff',
    lyricColorJP: '#ffffff',
    // Thick deep-navy outline (MV ink) + subtle sky-blue bloom for the "poster print" feel.
    lyricShadowEN: `
      -3px -3px 0 #0d2540,
      3px -3px 0 #0d2540,
      -3px 3px 0 #0d2540,
      ...
      0 -3px 0 #0d2540,
      0 3px 0 #0d2540,
      0 6px 0 rgba(13, 37, 64, 0.55),
      0 0 22px rgba(94, 184, 231, 0.35)
    `,
    lyricShadowJP: `
      -3px -3px 0 #0d2540,
      3px -3px 0 #0d2540,
      ...
      0 -3px 0 #0d2540,
      0 3px 0 #0d2540,
      0 7px 0 rgba(13, 37, 64, 0.6),
      0 0 26px rgba(94, 184, 231, 0.4)
    `,
  };
  ...
  // Bright saturated colors tuned for white-on-navy chunks on sky-blue.
  // Overwrite unconditionally (not `||`) so re-injection picks up palette edits.
  window.__wordAlign = window.__wordAlign && window.__wordAlign.data
    ? Object.assign(window.__wordAlign, { colors: ['#ffe94a','#76e6ff','#ff7ab6','#8cff9a','#ffae4a','#d9b4ff'] })
    : { colors: ['#ffe94a','#76e6ff','#ff7ab6','#8cff9a','#ffae4a','#d9b4ff'], data: {} };
  ...
  style.textContent = `
    ...
    /* ==========================================================================
       KYOUFUU ALL BACK — single-song overrides
       Hide the setlist/plain panels entirely (user asked for lyrics-only).
       Restyle lyrics to echo the MV's chunky sans-slam aesthetic.
    ========================================================================== */
    .ko-setlist, .ko-plain { display: none !important; }

    /* Stack lyric block higher — 66% default pushes into Gura's feet on vertical
       framing; 72% lets her body stay readable above the text. */
    #ko-lyrics .ko-slot { gap: 18px; }

    /* JP line: massive chunky sans, tight line-height so ruby doesn't balloon it */
    #ko-lyrics .ko-line-jp {
      font-family: "Zen Kaku Gothic New", "M PLUS 1", system-ui, sans-serif;
      font-weight: 900;
      font-size: 72px;
      line-height: 2.0;
      letter-spacing: 0.02em;
    ...
    /* Gloss labels — saturated colors, chunky sans, tucked above JP */
    #ko-lyrics .ko-line-jp rt {
    ...
    /* EN line: same chunky sans weight as JP — unified aesthetic */
    #ko-lyrics .ko-line-en {
      font-family: "Rubik", system-ui, sans-serif;
      font-weight: 900;
      font-size: 50px;
    ...
    /* ----- SIGNATURE FEATURE: slam-in cascade -----
       Each JP/EN chunk scales in with a staggered delay, mimicking how the
       original 強風オールバック MV slams chunky text onto screen.
       Animation fires fresh every time the colorizer rewrites innerHTML,
       which happens on every new lyric line. */
    @keyframes ko-slam-jp {
      0%   { transform: translateY(-14px) scale(1.08); opacity: 0; filter: blur(2px); }
      55%  { transform: translateY(2px)   scale(0.97); opacity: 1; filter: blur(0);   }
      100% { transform: translateY(0)     scale(1);    opacity: 1; filter: blur(0);   }
    }
    @keyframes ko-slide-en {
      0%   { transform: translateY(8px); opacity: 0; }
      100% { transform: translateY(0);   opacity: 1; }
    }

    /* JP colored chunks carry the slam */
    #ko-lyrics .ko-line-jp > span {
      display: inline-block;
      animation: ko-slam-jp 0.32s cubic-bezier(0.16, 0.8, 0.26, 1.05) backwards;
      transform-origin: 50% 70%;
    }
    /* Cascade delays — first chunk lands fastest, later chunks trail */
    #ko-lyrics .ko-line-jp > span:nth-child(1) { animation-delay: 0s;    }
    #ko-lyrics .ko-line-jp > span:nth-child(2) { animation-delay: 0.05s; }
    #ko-lyrics .ko-line-jp > span:nth-child(3) { animation-delay: 0.10s; }
    #ko-lyrics .ko-line-jp > span:nth-child(4) { animation-delay: 0.15s; }
    #ko-lyrics .ko-line-jp > span:nth-child(5) { animation-delay: 0.20s; }
    #ko-lyrics .ko-line-jp > span:nth-child(6) { animation-delay: 0.25s; }
    #ko-lyrics .ko-line-jp > span:nth-child(7) { animation-delay: 0.30s; }
    #ko-lyrics .ko-line-jp > span:nth-child(n+8) { animation-delay: 0.35s; }

    /* EN slides up a beat after JP lands */
    #ko-lyrics .ko-line-en > span {
      display: inline-block;
      animation: ko-slide-en 0.28s cubic-bezier(0.2, 0.8, 0.3, 1) 0.22s backwards;
    }

    /* Ruby gloss rides with its parent span */
    #ko-lyrics .ko-line-jp ruby { display: inline; }
    #ko-lyrics .ko-line-jp ruby rt { display: block; }
  `;
